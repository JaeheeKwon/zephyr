...
/* IS31FL3731 page specific register definitions */
...
#define IS31FL3731_ROW_COUNT 12
#define IS31FL3731_COL_COUNT 16
#define IS31FL3731_MAX_LED (IS31FL3731_ROW_COUNT * IS31FL3731_COL_COUNT)
...
struct is31fl3731_data {
	...
	uint8_t scratch_buf[IS31FL3731_MAX_LED + 1];
	/* LED config reg state, IS31FL3731 conf reg is write only */
	uint8_t conf_reg;
};

/* Selects target register page for IS31FL3731. After setting the
...
static int is31fl3731_led_set_brightness(const struct device *dev, uint32_t led, uint8_t value)
{
	...
	/* Configure PWM mode */
	ret = is31fl3731_select_page(dev, CMD_SEL_PWM);
	if (ret < 0) {
		return ret;
	}
...
static int is31fl3731_led_write_channels(const struct device *dev, uint32_t start_channel,
...
static int is31fl3731_init(const struct device *dev)
{
	...
	memset(data->scratch_buf + 1, 0xFF, (IS31FL3731_MAX_LED / 8));
	ret = is31fl3731_select_page(dev, CMD_SEL_LED);
	if (ret < 0) {
		return ret;
	}

	return i2c_write_dt(&config->bus, data->scratch_buf,
			(IS31FL3731_MAX_LED / 8) + 1);
}

/* Custom IS31FL3731 specific APIs */

/**
 * @brief Blanks IS31FL3731 LED display.
 *
 ...
 * @return 0 on success, or negative value on error.
 */
int is31fl3731_current_limit(const struct device *dev, uint8_t limit)
{
	const struct is31fl3731_config *config = dev->config;
	int ret;
